...
bool MainWindow::readJSON(unsigned char *key, unsigned char *nonce)
{
...
//    qDebug() << "***decryptedBytes" << decryptedBytes;
    int ret_code = doDecrypt(encryptedBytes, decryptedBytes, key, nonce);
...
int MainWindow::doDecrypt(const QByteArray& encryptedBytes, QByteArray& decryptedBytes, unsigned char *key, unsigned char *nonce) {
    // Вывод ключа для отладки


    qDebug() << "key " << QByteArray(reinterpret_cast<char*>(key), 32).toHex();
    qDebug() << "nonce " << QByteArray(reinterpret_cast<char*>(nonce), 12).toHex();

    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!EVP_DecryptInit_ex(ctx, EVP_chacha20(), NULL, key, nonce)) {
        qDebug() << "Error initializing decryption";
    ...
    if (!EVP_DecryptFinal_ex(ctx, decrypted_buf, &tmplen)) {
        qDebug() << "Error finalizing decryption";
        EVP_CIPHER_CTX_free(ctx);
        return 1;
    }
